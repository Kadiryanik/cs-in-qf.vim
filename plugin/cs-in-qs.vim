...
function! GetCS_In_QF (type) abort
	let l:cmd = 'cs find ' . a:type . ' ' . expand((a:type =~ '[if]') ? "<cfile>" : "<cword>")

	if strridx(&cscopequickfix, a:type) < 0
		echo "cscopequickfix option doesn't contain '" . a:type . "' option\n\n"
		execute l:cmd
		return
	endif
	" clear qflist
...
command! -nargs=1 Qcs call GetCS_In_QF(<f-args>)
